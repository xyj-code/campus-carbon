...
export function getUserTotalCarbon(username) {
  return request(`/project/carbon-total?username=${username}`);
}

// 用户已解锁项目列表
export function getUserProjectList(username) {
  return request(`/project/my-projects?username=${username}`);
}

// 检查并自动解锁达标项目（POST，query param）
export function checkAndUnlock(username) {
  return request(`/project/check-unlock?username=${username}`, { method: 'POST' });
}

// 项目详情
export function getProjectDetail(id) {
  return request(`/project/detail/${id}`);
}
